export interface Institution {
  name: string;
  type: "hospital" | "centro-medico" | "clinica" | "spa-medico";
  city: string;
  state: string;
  yearFounded: string;
  technicalMedicalDirector: string;
  crm?: string;
  complexityLevel: string;
  specialties: string;
  services?: string;
  beds: string;
  employees: string;
  website: string;
  socialMedia: string;
  socialProjects: string;
  image?: string;
}

const PLACEHOLDER = "Informação institucional em atualização";

export const hospitals: Institution[] = [
  {
    name: "Hospital Adventista Belém",
    type: "hospital",
    city: "Belém",
    state: "PA",
    yearFounded: "1953",
    technicalMedicalDirector: "Markus Barcellos de Albuquerque",
    crm: "CRM 4572/PA",
    complexityLevel: "Alta complexidade",
    specialties: "Alergologia, Alergologia Pediatra, Anestesiologia, Angiologia/Cirurgia Vascular, Arritmologia, Cabeça e Pescoço, Cardiologia, Cardiologia Pediatra, Cirurgião Cardíaco, Cirurgia Pediátrica, Cirurgia Plástica, Cirurgia Buco Maxilo, Cirurgia Oncológica, Cirurgia Geral, Cirurgião Torácico, Clínica Médica, Dermatologia, Dermatopediatra, Endocrinologia, Fonoaudiologia, Gastroenterologia Clínica, Gastroenterologia Pediatra, Geriatria, Ginecologia/Obstetrícia, Hematologia, Hepatologia, Infectologia, Laparoscopia e Histeroscopia Ginecológica, Mastologia, Marca-passo/Arritmologia, Neurologista Clínico, Neuropediatra, Neurocirurgia Pediátrica, Nefrologia, Nefropediatra, Nutrição, Nutrologia, Ortopedia/Traumatologia, Ortopedista Pediátrico/Traumatologia, Oncologista Clínico/Oncologia, Oncologia Pélvica, Onco-hematologia, Oftalmologia, Oftalmologia Pediatra, Otorrinolaringologia, Pediatria, Pneumologia, Proctologia, Pneumopediatria, Neurocirurgião, Psicologia, Psiquiatria, Reumatologia, Urologia",
    services: "Emergência, cirurgia, internação, exames diagnósticos e tratamento de doenças crônicas",
    beds: "190",
    employees: "1.650",
    website: "https://www.hab.org.br/",
    socialMedia:
      "Instagram: https://www.instagram.com/hospitalbelem/ | LinkedIn: https://www.linkedin.com/company/hospitalbelem",
    socialProjects: "Comunidade Encontro Vida - https://www.encontrovida.com.br/nossacomunidade",
  },
  {
    name: "Hospital Adventista Barcarena",
    type: "hospital",
    city: "Barcarena",
    state: "PA",
    yearFounded: "2021",
    technicalMedicalDirector: "Markus Barcellos de Albuquerque",
    crm: "CRM 4572/PA",
    complexityLevel: "Alta complexidade",
    specialties:
      "Clínico Geral, Pediatria, Ortopedia, Ginecologia, Obstetrícia, Urologia, Geriatria, Cardiologia, Cirurgia Geral, Gastrologia, Proctologia, Cirurgia Vascular, Otorrinolaringologia, Oftalmologia, Neurocirurgia, Bucomaxilo",
    services: "Pronto Atendimento Infantil e Adulto, Internação, Consultórios Médicos, Laboratório de Análises Clínicas, Diagnóstico por Imagem (Raio X, Tomografia, Mamografia, Ressonância Magnética, Ultrassonografia, Ecocardiograma, MAPA, Holter, Teste Ergométrico, ECG, EEG, Endoscopia, Colonoscopia, Retosigmoidoscopia)",
    beds: "12",
    employees: "180",
    website: PLACEHOLDER,
    socialMedia: PLACEHOLDER,
    socialProjects: PLACEHOLDER,
  },
  {
    name: "Hospital Adventista Pênfigo",
    type: "hospital",
    city: "Campo Grande - Matriz",
    state: "MS",
    yearFounded: "1950",
    technicalMedicalDirector: "Karin Kiefer Martins",
    crm: "CRM 4407/MS",
    complexityLevel: "Alta complexidade",
    specialties: "Cirurgia Geral, Cardiologia, Cirurgia Cabeça e Pescoço, Cirurgia Plástica, Cirurgia Torácica, Cirurgia Vascular, Clínica Geral, Coloproctologia, Dermatologia, Gastroenterologia, Ginecologia, Hepatologia, Neurocirurgia Pediátrica e Adulto, Neurologia, Oftalmologia, Oncologia, Ortopedia/Traumatologia, Otorrinolaringologia, Pediatria, Pneumologia, Proctologia, Urologia, Vascular",
    services: "Clínica médica, obstetrícia, cirurgias ginecológicas, ortopédicas, vasculares, plásticas e gerais; transplantes de fígado e córneas",
    beds: "136",
    employees: "700",
    website: PLACEHOLDER,
    socialMedia: PLACEHOLDER,
    socialProjects: PLACEHOLDER,
  },
  {
    name: "Hospital Adventista Manaus",
    type: "hospital",
    city: "Manaus",
    state: "AM",
    yearFounded: "1978",
    technicalMedicalDirector: "Francisco Mateus João",
    crm: "CRM 4870/AM",
    complexityLevel: "Alta complexidade",
    specialties: "Cardiologia, Arritmologia, Cardiologia Pediátrica, Ortopedia (Coluna, Joelho, Ombro, Mão, Quadril, Pé adulto e infantil), Ginecologia, Obstetrícia, Neurologia, Pediatria, Endocrinologia Pediátrica, Neuropediatria, Clínica Médica",
    services: "Centro Cirúrgico, Centro de Hemodinâmica, Centro Cardiológico, Centro Ortopédico, Centro Ginecológico/Obstétrico, Centro Neurológico, Centro de Radiodiagnóstico, CTI Adulto, UTI Pediátrica, UTI Neonatal, Internação Pediátrica, Maternidade (parto normal, cesárea, emergência obstétrica 24h), Pronto Atendimento 24h (clínico, ortopédico, pediátrico, ginecológico/obstétrico), Hemodinâmica/Cardiologia Intervencionista (cateterismo, angioplastia, ablação, TAVI, marcapasso, embolização, Protocolo AVC), Diagnóstico por Imagem (densitometria, mamografia, radiologia, ultrassonografia, ressonância magnética, angioressonância, tomografia, angiotomografia), Exames Cardiológicos (ecocardiograma, teste ergométrico, ECG, EEG, MAPA 24h, Holter 24h), Laboratório de Análises Clínicas (bacteriologia, bioquímica, hematologia, coagulação, imunologia, parasitologia, urianálise, biologia molecular, anatomopatológico, imuno-histoquímico, teste do pezinho)",
    beds: "160",
    employees: "1.577",
    website: PLACEHOLDER,
    socialMedia: PLACEHOLDER,
    socialProjects: PLACEHOLDER,
  },
  {
    name: "Hospital Adventista São Paulo",
    type: "hospital",
    city: "São Paulo",
    state: "SP",
    yearFounded: "1942",
    technicalMedicalDirector: "Douglas Charpinel",
    crm: "CRM 157752/SP",
    complexityLevel: "Alta complexidade",
    specialties: "Oncologia, Hematologia, Urologia, Neurologia, Neurocirurgia, Cardiologia, Ortopedia",
    services: "Centro de Diagnósticos, Endoscopia, Balão Intragástrico, Centro Cardiológico, Centro Cirúrgico, UTI, Unidade Semi-intensiva, Exames Laboratoriais e de Imagem",
    beds: "105",
    employees: "540",
    website: PLACEHOLDER,
    socialMedia: PLACEHOLDER,
    socialProjects: PLACEHOLDER,
  },
  {
    name: "Hospital Adventista Silvestre",
    type: "hospital",
    city: "Rio de Janeiro",
    state: "RJ",
    yearFounded: "1942",
    technicalMedicalDirector: "Ranieri Carvalho Leitão",
    crm: "CRM 649619/RJ",
    complexityLevel: "Alta complexidade",
    specialties: "Oncologia, Terapia Intensiva (UTI), Nutrição",
    services: "Laboratório de Análises Clínicas, Diagnóstico por Imagem (Raio-X, Tomografia, Ecocardiograma)",
    beds: "95",
    employees: "1.054",
    website: PLACEHOLDER,
    socialMedia: PLACEHOLDER,
    socialProjects: PLACEHOLDER,
  },
  {
    name: "Hospital Adventista Itaboraí",
    type: "hospital",
    city: "Itaboraí",
    state: "RJ",
    yearFounded: "2014",
    technicalMedicalDirector: "Guillermo Alberto Siado Camargo",
    crm: "CRM 107217-0/RJ",
    complexityLevel: "Alta complexidade — Reconhecimentos: Top Performer e UTI Eficiente (2023 e 2024)",
    specialties:
      "Anestesiologia, Cardiologia, Cirurgia Geral, Cirurgia Torácica, Cirurgia Oncológica, Dermatologia, Endocrinologia, Gastroenterologia, Hematologia, Neurologia, Neurocirurgia, Cirurgia e Traumatologia Bucomaxilofacial, Oftalmologia, Psicologia, Psiquiatria, Ortopedia e Traumatologia, Medicina Intensiva, Urologia, Cirurgia Bariátrica, Fisioterapia, Otorrinolaringologia, Nefrologia",
    services: "2 salas cirúrgicas, UTI, internação",
    beds: "20",
    employees: "110",
    website: PLACEHOLDER,
    socialMedia: PLACEHOLDER,
    socialProjects: PLACEHOLDER,
  },
];

export const medicalCenters: Institution[] = [
  {
    name: "Centro Médico Adventista Curitiba",
    type: "centro-medico",
    city: "Curitiba",
    state: "PR",
    yearFounded: "1994",
    technicalMedicalDirector: "Felipe Calderon Scarin",
    crm: "CRM 51954/PR",
    complexityLevel: "Atenção ambulatorial especializada",
    specialties: "Alergia e Imunologia, Angiologia, Biomedicina Estética, Cardiologia, Cirurgia Geral, Cirurgia Plástica, Cirurgia Vascular, Clínica Médica, Coloproctologia, Dermatologia, Endocrinologia e Metabologia, Endocrinologia Pediátrica, Fisioterapia",
    services: "Centro Médico, Laboratório de Análises Clínicas, Ecocardiografia (Ecocardiograma com Doppler)",
    beds: "N/A",
    employees: "65",
    website: PLACEHOLDER,
    socialMedia: PLACEHOLDER,
    socialProjects: PLACEHOLDER,
  },
  {
    name: "Centro Médico Adventista Porto Alegre",
    type: "centro-medico",
    city: "Porto Alegre",
    state: "RS",
    yearFounded: "1989",
    technicalMedicalDirector: "Eli Arado Filho",
    crm: "CRM 51866/RS",
    complexityLevel: "Atenção ambulatorial especializada",
    specialties: "Anestesiologia/Medicina da Dor, Audiologia, Cardiologia, Cirurgia do Aparelho Digestivo, Cirurgia Geral, Cirurgia Plástica, Cirurgia Vascular, Clínica Médica, Coloproctologia, Dermatologia, Endocrinologia e Metabologia, Fisioterapia, Fisioterapia Pélvica",
    services: "Centro Médico, Laboratório de Análises Clínicas, Audiometria (tonal, vocal, impedanciometria, teste de prótese auditiva), Ecocardiografia (Ecocardiograma com Doppler)",
    beds: "N/A",
    employees: "35",
    website: PLACEHOLDER,
    socialMedia: PLACEHOLDER,
    socialProjects: PLACEHOLDER,
  },
  {
    name: "Centro Médico Adventista Botafogo",
    type: "centro-medico",
    city: "Rio de Janeiro",
    state: "RJ",
    yearFounded: PLACEHOLDER,
    technicalMedicalDirector: "Mariana Gomes Fernandes",
    crm: "CRM 77936-9/RJ",
    complexityLevel: "Atenção ambulatorial e diagnóstica",
    specialties: "Angiologia/Cirurgia Vascular, Arritmologia, Bucomaxilo, Cardiologia, Cirurgia Geral, Cirurgia Oncológica, Cirurgia Bariátrica, Cirurgia Plástica, Cirurgia Torácica, Cirurgia Vascular, Clínica Médica, Clínica da Dor, Dermatologia, Ecocardiograma, Endocrinologia, Endoscopia/Colonoscopia, Fisioterapia, Fonoaudiologia, Gastroenterologia, Geriatria, Ginecologia/Mastologia, Hematologia, Nefrologia, Neurologia, Nutrição, Oftalmologia, Ortopedia/Traumatologia, Pediatria, Pneumologia, Psicologia, Psiquiatria, Reumatologia, Urologia",
    beds: PLACEHOLDER,
    employees: PLACEHOLDER,
    website: PLACEHOLDER,
    socialMedia: PLACEHOLDER,
    socialProjects: PLACEHOLDER,
  },
  {
    name: "Centro Médico Adventista Capão Redondo",
    type: "centro-medico",
    city: "São Paulo",
    state: "SP",
    yearFounded: PLACEHOLDER,
    technicalMedicalDirector: PLACEHOLDER,
    complexityLevel: "Atenção ambulatorial",
    specialties: PLACEHOLDER,
    beds: "N/A",
    employees: PLACEHOLDER,
    website: PLACEHOLDER,
    socialMedia: PLACEHOLDER,
    socialProjects: PLACEHOLDER,
  },
  {
    name: "Centro Médico Adventista Engenheiro Coelho",
    type: "centro-medico",
    city: "Engenheiro Coelho",
    state: "SP",
    yearFounded: PLACEHOLDER,
    technicalMedicalDirector: "Luana das Graças de Oliveira",
    crm: "CRM 194458/SP",
    complexityLevel: "Atenção ambulatorial",
    specialties: "Cardiologia, Clínica Médica Geral, Endocrinologia, Dermatologia, Ginecologia, Urologia, Otorrinolaringologia, Oftalmologia, Gastroenterologia, Ortopedia, Psiquiatria, Pediatria, Fisioterapia, Fonoaudiologia, Nutrição, Psicologia",
    services: "Centro de Reabilitação e Fisioterapia (reabilitação ortopédica, neurológica, RPG, piscina terapêutica, hidroterapia, corrente russa, Pilates), Terapias e Bem-Estar (massagem relaxante, antiestresse, clássica, drenagem linfática, esfoliação corporal), Estética Facial (drenagem linfática facial, hidratação, limpeza de pele, máscara de argila), Estética Corporal (redução de medidas, estrias, celulite), Programas de Saúde (Imunidade/Detox, Fitness, Saúde emocional, Relax, Reabilitação motora), Exames (ultrassonografia, eletrocardiograma, endoscopia, colonoscopia, teste ergométrico, MAPA 24h, Holter 24h, audiometria, impedanciometria, processamento auditivo, reabilitação labiríntica, exames laboratoriais)",
    beds: "N/A",
    employees: PLACEHOLDER,
    website: PLACEHOLDER,
    socialMedia: PLACEHOLDER,
    socialProjects: PLACEHOLDER,
  },
];

export const clinics: Institution[] = [
  {
    name: "Clínica Adventista Vila Andrade",
    type: "clinica",
    city: "São Paulo",
    state: "SP",
    yearFounded: PLACEHOLDER,
    technicalMedicalDirector: PLACEHOLDER,
    complexityLevel: "Atenção Primária à Saúde (APS)",
    specialties: "Medicina de Família, Enfermagem de Família, Psicologia, Nutrição, Fisioterapia",
    services: "Atendimento baseado em APS com assistência médica e multiprofissional, foco em cuidado integral, longitudinal e contínuo. PA Digital 24/7. Não inclui urgência/emergência complexa ou cirúrgica — casos identificados são estabilizados e encaminhados para hospital de alta complexidade.",
    beds: "N/A",
    employees: PLACEHOLDER,
    website: PLACEHOLDER,
    socialMedia: PLACEHOLDER,
    socialProjects: PLACEHOLDER,
  },
  {
    name: "Clínica Adventista Marco",
    type: "clinica",
    city: "Belém",
    state: "PA",
    yearFounded: "2026",
    technicalMedicalDirector: PLACEHOLDER,
    complexityLevel: "Atenção Primária à Saúde (APS)",
    specialties: "Medicina de Família, Enfermagem de Família, Psicologia, Nutrição, Fisioterapia",
    services: "Atendimento baseado em APS com assistência médica e multiprofissional, foco em cuidado integral, longitudinal e contínuo. PA Digital 24/7. Não inclui urgência/emergência complexa ou cirúrgica — casos identificados são estabilizados e encaminhados para o Hospital Adventista Belém.",
    beds: "N/A",
    employees: PLACEHOLDER,
    website: PLACEHOLDER,
    socialMedia: PLACEHOLDER,
    socialProjects: PLACEHOLDER,
  },
];

export const medicalSpas: Institution[] = [
  {
    name: "Spa Médico Adventista Cevisa",
    type: "spa-medico",
    city: PLACEHOLDER,
    state: PLACEHOLDER,
    yearFounded: PLACEHOLDER,
    technicalMedicalDirector: "Luana das Graças de Oliveira",
    crm: "CRM 194458/SP",
    complexityLevel: "Medicina do estilo de vida e reabilitação",
    specialties: "Medicina do estilo de vida e reabilitação",
    services: "Hidroterapia, fisioterapia, terapias naturais, massoterapia, pilates, atividade física assistida",
    beds: "40 quartos",
    employees: PLACEHOLDER,
    website: PLACEHOLDER,
    socialMedia: PLACEHOLDER,
    socialProjects: PLACEHOLDER,
  },
  {
    name: "Spa Médico Adventista CVS",
    type: "spa-medico",
    city: PLACEHOLDER,
    state: PLACEHOLDER,
    yearFounded: PLACEHOLDER,
    technicalMedicalDirector: "Jose Januário de Farias Neto",
    crm: "CRM 14107/MS",
    complexityLevel: "Medicina do estilo de vida e reabilitação",
    specialties: "Medicina do estilo de vida e reabilitação",
    services: "Hidroterapia, massoterapia, estética terapêutica, atividade física assistida",
    beds: "10 quartos",
    employees: PLACEHOLDER,
    website: PLACEHOLDER,
    socialMedia: PLACEHOLDER,
    socialProjects: PLACEHOLDER,
  },
];

export const allInstitutions = [...hospitals, ...medicalCenters, ...clinics, ...medicalSpas];
